...
export default function GanttChart({ sites, dense = false }) {
  const { start, end } = nextMonthsRange(3);
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold">Gantt-Übersicht (nächste 3 Monate)</h2>
      ...
      <div className="relative overflow-x-auto">
        ...
        >
          ...
          {sites.map((site) => {
            const { offset, length } = clampDays(site.startDate, site.endDate, start, end);
              ...
              <>
                ...
                <div key={`${site.id}-bar`} className="relative flex h-16 items-center rounded-lg bg-slate-950/30">
                  <div
                    ...
                    title={`${site.name}: ${dayjs(site.startDate).format('DD.MM.')} - ${dayjs(site.endDate).format('DD.MM.')}`}
                  />
                </div>
              </>
            );
